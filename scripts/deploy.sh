...
IMAGE_NAME="${IMAGE_NAME:-etrade}"
RUNTIME_VOLUME="${RUNTIME_VOLUME:-etrade-runtime}"
PORT="${PORT:-8080}"
CANDIDATE_PORT="${CANDIDATE_PORT:-18080}"
PUBLIC_URL="${PUBLIC_URL:-https://edelivery.by}"
EXPECTED_SITE_URL="${EXPECTED_SITE_URL:-https://edelivery.by}"
ENV_FILE="${ENV_FILE:-.env}"
APP_NETWORK="${APP_NETWORK:-etrade-net}"
GIT_REF="${GIT_REF:-origin/main}"
SKIP_GIT_SYNC="${SKIP_GIT_SYNC:-0}"
BUILD_NO_CACHE="${BUILD_NO_CACHE:-0}"
DOCKER_CACHE_DIR="${DOCKER_CACHE_DIR:-/root/.cache/etrade-buildx}"
...
SMOKE_REQUIRE_BITRIX="${SMOKE_REQUIRE_BITRIX:-1}"
...
SMOKE_REQUIRE_BITRIX="$SMOKE_REQUIRE_BITRIX" \
RUNTIME_PREPARE_TOKEN="$RUNTIME_PREPARE_TOKEN" \
