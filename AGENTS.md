...
## Admin Access Policy (Mandatory)
- На публичных страницах сайта запрещено размещать любые кнопки/ссылки/пункты меню, ведущие на `/admin` (включая `https://edelivery.by/admin`).
- Админ-панель должна быть доступна только при прямом вводе адреса `https://edelivery.by/admin`.
- Запрещено использовать браузерные всплывающие окна авторизации (HTTP Basic Auth popup) в проекте.
- Для входа в админку использовать только встроенную страницу/форму авторизации сайта (`/admin/login`).
